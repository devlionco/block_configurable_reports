  ...
  */
	...
	if($reports){
		...
		foreach($reports as $r){
			...
			$editcell = '';
			$editcell .= '<a title="'.$stredit.'"  href="editcomp.php?id='.$r->id.'&comp=customsql&courseid='.$courseid.'"><img src="'.$OUTPUT->image_url('/i/db').'" class="icon" alt="'.$stredit.'" /></a>&nbsp;&nbsp;';
			$editcell .= '<a title="'.$strdelete.'"  href="editreport.php?id='.$r->id.'&amp;delete=1&amp;sesskey='.$USER->sesskey.'"><img src="'.$OUTPUT->image_url('/t/delete').'" class="icon" alt="'.$strdelete.'" /></a>&nbsp;&nbsp;';

			if (!empty($r->visible)) {
				$editcell .= '<a title="'.$strhide.'" href="editreport.php?id='.$r->id.'&amp;hide=1&amp;sesskey='.$USER->sesskey.'">'.'<img src="'.$OUTPUT->image_url('/t/hide').'" class="icon" alt="'.$strhide.'" /></a> ';}
			else {
				$editcell .= '<a title="'.$strshow.'" href="editreport.php?id='.$r->id.'&amp;show=1&amp;sesskey='.$USER->sesskey.'">'.'<img src="'.$OUTPUT->image_url('/t/show').'" class="icon" alt="'.$strshow.'" /></a> ';
			}
			$editcell .= '<a title="'.$strcopy.'" href="editreport.php?id='.$r->id.'&amp;duplicate=1&amp;sesskey='.$USER->sesskey.'"><img src="'.$OUTPUT->image_url('/t/copy').'" class="icon" alt="'.$strcopy.'" /></a>&nbsp;&nbsp;';
			$editcell .= '<a title="'.$strexport.'" href="export.php?id='.$r->id.'&amp;sesskey='.$USER->sesskey.'"><img src="'.$OUTPUT->image_url('/i/export').'" class="icon" alt="'.$strexport.'" /></a>&nbsp;&nbsp;';
            $editcell .= '<a title="'.$strsettings.'"  href="editreport.php?id='.$r->id.'"><img src="'.$OUTPUT->image_url('/i/settings').'" class="icon" alt="'.$stredit.'" /></a>&nbsp;&nbsp;';
